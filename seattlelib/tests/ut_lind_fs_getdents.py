...
lind_test_server.get_fscall_obj(-1).mkdir_syscall('/bar/bap',0)

# Create a new file...
fd = lind_test_server.get_fscall_obj(-1).open_syscall('/bar/bam',O_CREAT,0)
lind_test_server.get_fscall_obj(-1).close_syscall(fd)

# Read the root directory...
rootfd = lind_test_server.get_fscall_obj(-1).open_syscall('/',0,0)
val = lind_test_server.get_fscall_obj(-1).getdents_syscall(rootfd, 100)
assert (val==[(3, 'bar', DT_DIR, 24), (1, '..', DT_DIR, 24),\
  (1, '.', DT_DIR, 24)]), "Found: %s"%(str(val))

# Read the /bar directory...
barfd = lind_test_server.get_fscall_obj(-1).open_syscall('/bar',0,0)

# The buffer size is given small, only few entries are read. 
val = lind_test_server.get_fscall_obj(-1).getdents_syscall(barfd, 80)
...
lind_test_server.get_fscall_obj(-1).close_syscall(rootfd)
lind_test_server.get_fscall_obj(-1).close_syscall(barfd)

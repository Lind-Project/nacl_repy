...
val = lind_test_server.getdents_syscall(rootfd, 10)
assert(val==[(3, 'bar', DT_DIR), (1, '..', DT_DIR), (1, '.', DT_DIR)]), "Found: %s"%(str(val))

barfd = lind_test_server.open_syscall('/bar',0,0)
val = lind_test_server.getdents_syscall(barfd, 10)
assert(val==[(6, 'bam', DT_REG), (4, 'baz', DT_DIR), (5, 'bap', DT_DIR), (1, '..', DT_DIR), (3, '.', DT_DIR)])

assert(lind_test_server.lseek_syscall(barfd,0,SEEK_SET) == 0)
assert(lind_test_server.getdents_syscall(barfd,1) == [(6, 'bam', DT_REG)])
assert(lind_test_server.getdents_syscall(barfd,1) == [(4, 'baz', DT_DIR)])

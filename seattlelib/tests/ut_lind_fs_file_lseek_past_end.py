...
assert(lind_test_server.get_fs_call(-1,"write_syscall")(myfd,'hello') == 5)

# seek past the end
assert(lind_test_server.get_fs_call(-1,"lseek_syscall")(myfd,10,SEEK_SET) == 10)

# write should succeed again (past the end)
assert(lind_test_server.get_fs_call(-1,"write_syscall")(myfd,'yoyoyo') == 6)

# seek to the start
assert(lind_test_server.get_fs_call(-1,"lseek_syscall")(myfd,0,SEEK_SET) == 0)

# read 20 bytes but get 16 (hello\0\0\0\0\0yoyoyo)
assert(lind_test_server.get_fs_call(-1,"read_syscall")(myfd,20)=='hello\0\0\0\0\0yoyoyo')

...
# bind to a socket
lind_test_server.get_fscall_obj(-1).bind_syscall(serversockfd, '127.0.0.1', 50431)
...
def do_server():
    ...
    fd = newsocketfd[2]
    lind_test_server.get_fscall_obj(-1).send_syscall(fd, 'jhasdfhjsa', 0)
    lind_test_server.get_fscall_obj(-1).netshutdown_syscall(fd, SHUT_RDWR)
...
lind_test_server.get_fscall_obj(-1).connect_syscall(clientsockfd, '127.0.0.1', 50431)
assert(lind_test_server.get_fscall_obj(-1).getpeername_syscall(clientsockfd) == \
       ('127.0.0.1', 50431))

...
if __name__ == '__main__':
    ...
    myfd = lind_test_server.get_fscall_obj(-1).open_syscall(TEST_FILENAME, \
                                      O_CREAT | O_EXCL | O_RDWR, S_IRWXA)

    # write should succeed
    assert(lind_test_server.get_fscall_obj(-1).write_syscall(myfd, 'hello there!') == 12)

    # seek to the beginning...
    assert(lind_test_server.get_fscall_obj(-1).lseek_syscall(myfd, 0, SEEK_SET) == 0)

    # read the first 5 bytes (hello)
    assert(lind_test_server.get_fscall_obj(-1).read_syscall(myfd, 5) == 'hello')

    # change it to hello world!
    assert(lind_test_server.get_fscall_obj(-1).write_syscall(myfd, ' world') == 6)

    # seek to the beginning again...
    assert(lind_test_server.get_fscall_obj(-1).lseek_syscall(myfd, 0, SEEK_SET) == 0)

    # and read it all...
    assert(lind_test_server.get_fscall_obj(-1).read_syscall(myfd, 100) == 'hello world!')

    lind_test_server.get_fscall_obj(-1).close_syscall(myfd)

    # Now make and remove a file:
    myfd = lind_test_server.get_fscall_obj(-1).open_syscall(TEST2_FILENAME, \
                                      O_CREAT | O_EXCL | O_RDWR, S_IRWXA)

    # write should succeed
    message = '================================================================================================================================'
    assert(lind_test_server.get_fscall_obj(-1).write_syscall(myfd, message) == len(message))

    lind_test_server.get_fscall_obj(-1).close_syscall(myfd)

    lind_test_server.get_fscall_obj(-1).unlink_syscall(TEST2_FILENAME)

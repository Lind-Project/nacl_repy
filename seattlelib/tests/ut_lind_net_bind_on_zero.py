...
clientsockfd2 = lind_test_server.get_net_call(-1,"socket_syscall")(AF_INET, SOCK_STREAM, 0)

#Bind & listen with backlog of 1, since I am creating only 1 client. 
lind_test_server.get_net_call(-1,"bind_syscall")(serversockfd, '0.0.0.0', 50300)
lind_test_server.get_net_call(-1,"listen_syscall")(serversockfd, 1)
...
def process_request():
  ...
  rip, rport, newsockfd = lind_test_server.get_net_call(-1,"accept_syscall")(serversockfd)  
  
  #Process first test...
  try:
    msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 100, MSG_PEEK)
    assert len(msg[2]) == 100, "Length expected 100 bytes to peek, but only" \
      + " got " + str(len(msg[2])) + " bytes."
    msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 100, 0)
  ...
  #Process second test...
  try:
    msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 20, 0)
    assert len(msg[2]) == 20, "Length expected 20 bytes to read, but only" \
      + " got " + str(len(msg[2])) + " bytes."
    msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 20, MSG_PEEK)
    assert len(msg[2]) == 20, "Length expected 20 bytes to peek, but only" \
      + " got " + str(len(msg[2])) + " bytes."
    msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 80, 0)
  ...
  try:
    for i in range(0,4):
      msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 10, MSG_PEEK)
    ...
    for i in range(0,4):
      ...
      assert len(msg[2]) == 20, "Length expected 20 bytes to peek, but only" \
        + " got " + str(len(msg[2])) + " bytes."
    for i in range(0,4):
      msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 30, MSG_PEEK)
      assert len(msg[2]) == 30, "Length expected 30 bytes to peek, but only" \
        + " got " + str(len(msg[2])) + " bytes."
    for i in range(0,4):
      msg = lind_test_server.get_net_call(-1,"recvfrom_syscall")(newsockfd, 40, MSG_PEEK)
  ...
  lind_test_server.get_fs_call(-1,"close_syscall")(newsockfd)
...
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd, "A" * 50, 0)
emultimer.sleep(0.1)

lind_test_server.get_fs_call(-1,"close_syscall")(clientsockfd)


lind_test_server.get_net_call(-1,"connect_syscall")(clientsockfd2, emulcomm.getmyip(), 50300)

#send each test with some delay, so that server processes each test cleanly.
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd2, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd2, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd2, "A" * 100, 0)
emultimer.sleep(0.1)
lind_test_server.get_net_call(-1,"send_syscall")(clientsockfd2, "A" * 50, 0)
emultimer.sleep(0.1)



#close the client & server sockets...  
lind_test_server.get_fs_call(-1,"close_syscall")(clientsockfd2)

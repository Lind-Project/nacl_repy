...
sockfd2 = lind_test_server.get_net_call(-1,"socket_syscall")(AF_INET, SOCK_STREAM, 0)

# let's set them to allow port reuse
lind_test_server.get_net_call(-1,"setsockopt_syscall")(sockfd, SOL_SOCKET, SO_REUSEPORT, 1)
lind_test_server.get_net_call(-1,"setsockopt_syscall")(sockfd2, SOL_SOCKET, SO_REUSEPORT, 1)

# should work...
lind_test_server.get_net_call(-1,"bind_syscall")(sockfd2,'127.0.0.1',50102)

# now let's try to listen on both... (should fail on the second)

lind_test_server.get_net_call(-1,"listen_syscall")(sockfd,10)

try:
  lind_test_server.get_net_call(-1,"listen_syscall")(sockfd2,10)

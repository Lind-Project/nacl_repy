...
def recvmessages():
  ...
  lind_test_server.get_fscall_obj(-1).bind_syscall(listensockfd,'127.0.0.1',50102)
  assert lind_test_server.get_fscall_obj(-1).recv_syscall(listensockfd,10000,0) == 'test',\
    "UDP recv test 1 failed."
  lind_test_server.get_fscall_obj(-1).recv_syscall(listensockfd,10000,0) == 'test2',\
...
# send a message
lind_test_server.get_fscall_obj(-1).sendto_syscall(sendsockfd,'test','127.0.0.1',50102,0)

# get another socket, bind and then send
sendsockfd2 = lind_test_server.get_fscall_obj(-1).socket_syscall(AF_INET, SOCK_DGRAM, 0)
lind_test_server.get_fscall_obj(-1).bind_syscall(sendsockfd2,'127.0.0.1',50992)
lind_test_server.get_fscall_obj(-1).sendto_syscall(sendsockfd2,'test2','127.0.0.1',50102,0)

sleep(0.1)
lind_test_server.get_fscall_obj(-1).close_syscall(sendsockfd2)
